...
case "\${1:-}" in
    ...
    list-panes)
        ...
        exit 1
        ;;
    switch-client|select-window|select-pane|kill-session)
        exit 0
        ;;
    *)
        exit 1
...
if ! grep -Fq "switch-client -t zeta" "$LOG_FILE"; then
    echo "Assertion failed: closing the current session should switch to the next inbox item" >&2
    sed -n '1,120p' "$LOG_FILE" >&2
    exit 1
fi
if ! grep -Fq "kill-session -t repo" "$LOG_FILE"; then
    echo "Assertion failed: close-target should still kill the selected session" >&2
    sed -n '1,120p' "$LOG_FILE" >&2
    exit 1
fi

echo "close target session next inbox regression checks passed"
